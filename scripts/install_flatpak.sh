...
# The package name will be modified, dropping the freedesktop branch in order
# to allow a comparison between package IDs
# Defaults of requisition:
#   $1 - Name of package
try_install_flatpak() {
	try=0
	while [ "$(flatpak list | grep --count "${1%%/*}")" -eq 0 ] && [ $try -lt 3 ]; do
		flatpak install flathub "$1" -y
	    try=$((try+1))
	done

	# In case of success, return 0. In case of failure, return 1.
	if [ "$(flatpak list | grep --count "${1%%/*}")" -eq 0 ]; then
		return 1
	else
		return 0
	fi
